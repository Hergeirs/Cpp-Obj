...
const string &  Name::getFirstName() const				// Getting the private names out in public.
{
	return firstName;
}
...
const bool Name::operator<(Name &pName) const
{
	if (toCase(pName.lastName)==toCase(lastName)) // ensuring sorting by firstname if lastname equal.
	{

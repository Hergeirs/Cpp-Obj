...
void Person::print(int n,int width) const
{
	cout << endl << "**********Person Info*********" << endl;
	if (n!=0)
	{
		cout << setw(width) << "Queue Nr: " << setw(width) << n << endl;
	}
	name.print();
...
void Person::operator = (Person &pPerson)
{
	name=pPerson.name;
	address=pPerson.address;
	skoNr=pPerson.skoNr;
	persNr=pPerson.persNr;
}
